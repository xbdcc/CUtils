 ...
 */
class RoundRectTextView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : CTextView(context, attrs, defStyleAttr) {
    ...
    init {
        val typedArray =
            context.obtainStyledAttributes(
                attrs,
                R.styleable.RoundRectTextViewView,
                defStyleAttr,
                0
            )
        color = typedArray.getColor(R.styleable.RoundRectTextViewView_solidColor, Color.BLUE)
        radius = typedArray.getDimension(R.styleable.RoundRectTextViewView_radius, 0f)
        topLeftRadius = typedArray.getDimension(R.styleable.RoundRectTextViewView_topLeftRadius, 0f)
        topRightRadius =
            typedArray.getDimension(R.styleable.RoundRectTextViewView_topRightRadius, 0f)
        bottomLeftRadius =
            typedArray.getDimension(R.styleable.RoundRectTextViewView_bottomLeftRadius, 0f)
        bottomRightRadius =
            typedArray.getDimension(R.styleable.RoundRectTextViewView_bottomRightRadius, 0f)
...
//        paint.textSize = 30f
//        canvas.drawText("fdfdgdgdgl", rect.width() / 2, rect.height() / 2, paint)
    }
